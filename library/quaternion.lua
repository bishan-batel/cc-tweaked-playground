...
function quaternion.fromAxisAngle(axis, angle)
  ...
  if type(axis) == "table" and (getmetatable(axis) or {}).__name ~= "vector" then
...
function quaternion.fromEuler(pitch, yaw, roll)
  ...
  yaw = yaw or 0
  roll = roll or 0
  return fromAxisAngle(vector.new(0, 1, 0), yaw) *
    fromAxisAngle(vector.new(1, 0, 0), pitch) *
    fromAxisAngle(vector.new(0, 0, 1), roll)
end
...
-- @export
---@return Quaternion
function quaternion.fromComponents(x, y, z, w)
...
function quaternion.fromMatrix(m)
  ...
  return quaternion.fromComponents(x, y, z, w)
end

function quaternion.fromShip()
  error(
    "This method is no longer required as of CC: VS 0.4.0, as the Ship API now provides quaternions directly.")
end
...
local Quaternion = {
  ...
  add = function(self, other)
    expect(1, self, "table")
    if (getmetatable(self) or {}).__name ~= "quaternion" then
      expect(1, self,
        "quaternion")
    end
  ...
  mul = function(self, other)
    ...
    if type(self) == "table" and (getmetatable(self) or {}).__name ~= "quaternion" and (getmetatable(self) or {}).__name ~= "vector" then
      expect(1, self, "quaternion", "vector", "number")
    end
    expect(2, other, "table", "number")
    if type(other) == "table" and (getmetatable(other) or {}).__name ~= "quaternion" and (getmetatable(other) or {}).__name ~= "vector" then
  ...
  div = function(self, other)
    expect(1, self, "table", "number")
    if type(self) == "table" and (getmetatable(self) or {}).__name ~= "quaternion" then
      expect(1, self, "quaternion", "number")
    end
    expect(2, other, "table", "number")
    if type(self) == "table" and (getmetatable(other) or {}).__name ~= "quaternion" then
  ...
  equals = function(self, other)
    ...
    expect(2, other, "table")
    if (getmetatable(other) or {}).__name ~= "quaternion" then
      expect(2, other,
        "quaternion")
    end
  ...
  inverse = function(self)
    ...
    return self:conjugate() /
      (self.a ^ 2 + self.v.x ^ 2 + self.v.y ^ 2 + self.v.z ^ 2)
  end,
  ...
  slerp = function(self, other, alpha)
    ...
    local a = self:copy()
    local b = other:copy()
    local cos_half_theta = a.a * b.a + a.v.x * b.v.x + a.v.y * b.v.y +
      a.v.z * b.v.z
  ...
  -- @usage q:isNan()
  isNan = function(self)
    return self.a ~= self.a or self.v.x ~= self.v.x or self.v.y ~= self.v.y or
      self.v.z ~= self.v.z
  end,
  ...
  isInf = function(self)
    local inf = 1 / 0
    return math.abs(self.a) == inf or math.abs(self.v.x) == inf or
      math.abs(self.v.y) == inf or math.abs(self.v.z) == inf
  end,
