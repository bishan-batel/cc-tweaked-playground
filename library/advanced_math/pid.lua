...
local function scalarStep(self, value, dt)
  ...
  if self.integral_min and self.integral_max then
    self.integral = math.max(self.integral_min,
      math.min(self.integral_max, self.integral))
  end
...
local function vectorStep(self, value, dt)
  ...
  if (getmetatable(value) or {}).__name ~= "vector" then
    expect(1, value,
      "vector")
  end
  expect(2, dt, "number", "nil")
...
local function quaternionStep(self, value, dt)
  expect(1, value, "table")
  if (getmetatable(value) or {}).__name ~= "quaternion" then
    expect(1, value,
      "quaternion")
  end
  expect(2, dt, "number", "nil")
...
local pid = {
  ...
  tostring = function(self)
    ...
    return string.format("%s PID {SP = %s, Kp = %d, Ki = %d, Kd=%d}", mode, sp,
