...
local function newBuffer(x, y, w, h)
  ...
  function buffer:drawTriangleInterp(x1, y1, x2, y2, x3, y3, c, outlineColor, d,
                                     z1, z2, z3)
    if x1 < 0 and x2 < 0 and x3 < 0 or y1 < 1 and y2 < 1 and y3 < 1 then return end
  ...
  function buffer:depthInterpolation(enabled)
    self.drawTriangle = enabled and self.drawTriangleInterp or
      self.drawTriangleNoInterp
    self:clear()
  end
...
function transforms.decimate(model, quality, mode)
  ...
  while #triangles > maxTriangles do
    ...
    for i = #candidateEdges, 1, -1 do
      local candidate = candidateEdges[i]
      local replaced1 = candidate.vA == smallestCandidate.vA or
        candidate.vA == smallestCandidate.vB
      local replaced2 = candidate.vB == smallestCandidate.vA or
        candidate.vB == smallestCandidate.vB
...
function transforms.toLoD(model, settings)
  ...
  for i = 1, LoDModel.variantCount do
    local quality = (1 - LoDModel.minQuality) * (1 - i / LoDModel.variantCount) +
      LoDModel.minQuality
  ...
  function LoDModel:update(object, camera)
    ...
    if variants then
      ...
      local targetQuality = math.min(1,
        math.max(LoDModel.minQuality, 1 / (d / LoDModel.qualityHalvingDistance)))
      local closestRaw = (LoDModel.variantCount + 1) -
        LoDModel.variantCount * (targetQuality - LoDModel.minQuality) /
...
function loadModelRaw(model)
  ...
  for i = 1, #model do
    ...
    local d1 = sqrt(polygon.x1 * polygon.x1 + polygon.y1 * polygon.y1 +
      polygon.z1 * polygon.z1)
    local d2 = sqrt(polygon.x2 * polygon.x2 + polygon.y2 * polygon.y2 +
      polygon.z2 * polygon.z2)
    local d3 = sqrt(polygon.x3 * polygon.x3 + polygon.y3 * polygon.y3 +
      polygon.z3 * polygon.z3)
...
local function newFrame(x, y, w, h)
  ...
  frame.camera[8] = 2 *
    math.atan(math.tan(rad(frame.FoV) * 0.5) / (frame.width / frame.height / 1.5))
  ...
  function frame:drawObject(object, camera, cameraAngles)
    ...
    local clippingEnabled = xCameraOffset * xCameraOffset +
      yCameraOffset * yCameraOffset + zCameraOffset * zCameraOffset <
    ...
    for i = 1, #depthPolygons do
      ...
      local x1, y1, dX1 = map3dTo2d(polygon[1] + xCameraOffset,
        polygon[2] + yCameraOffset, polygon[3] + zCameraOffset)
      if dX1 > 0.00010000001 then
        local x2, y2, dX2 = map3dTo2d(polygon[4] + xCameraOffset,
          polygon[5] + yCameraOffset, polygon[6] + zCameraOffset)
        if dX2 > 0.00010000001 then
          local x3, y3, dX3 = map3dTo2d(polygon[7] + xCameraOffset,
            polygon[8] + yCameraOffset, polygon[9] + zCameraOffset)
          if dX3 > 0.00010000001 then
            if polygon[10] or (x2 - x1) * (y3 - y2) - (y2 - y1) * (x3 - x2) < 0 then
              buff:drawTriangle(x1, y1, x2, y2, x3, y3, polygon[11], polygon[12],
                depth, dX1, dX2, dX3)
            end
          elseif clippingEnabled then
            ...
            local x1, y1, dX1, dY1, dZ1 = map3dTo2dFull(polygon[1], polygon[2],
              polygon[3])
            local x2, y2, dX2, dY2, dZ2 = map3dTo2dFull(polygon[4], polygon[5],
              polygon[6])
            local x3, y3, dX3, dY3, dZ3 = map3dTo2dFull(polygon[7], polygon[8],
              polygon[9])
            ...
            local renderOffsetX, sXFactor, renderOffsetY, sYFactor =
              renderOffsetX, sXFactor, renderOffsetY, sYFactor
            ...
            if polygon[10] or (x2 - x1) * (AY - y2) - (y2 - y1) * (AX - x2) < 0 then
              buff:drawTriangle(x1, y1, x2, y2, AX, AY, polygon[11], polygon[12],
                depth, dX1, dX2, 0.0001)
              ...
              buff:drawTriangle(BX, BY, x2, y2, AX, AY, polygon[11], polygon[12],
                depth, 0.0001, dX2, 0.0001)
            end
          end
        elseif clippingEnabled then
          local function map3dTo2dFull(x, y, z)
          ...
          if dX3 > 0.00010000001 then
            ...
            if polygon[10] or (AX - x1) * (y3 - AY) - (AY - y1) * (x3 - AX) < 0 then
              buff:drawTriangle(x1, y1, AX, AY, x3, y3, polygon[11], polygon[12],
                depth, dX1, 0.0001, dX3)
              ...
              local BX = (newPosAZ * 10000) * sXFactor + renderOffsetX
              local BY = (newPosAY * 10000) * sYFactor + renderOffsetY
              buff:drawTriangle(BX, BY, AX, AY, x3, y3, polygon[11], polygon[12],
                depth, 0.0001, 0.0001, dX3)
            end
          else
            -- 1 0 0
            ...
            if polygon[10] or (AX - x1) * (BY - AY) - (AY - y1) * (BX - AX) < 0 then
              buff:drawTriangle(x1, y1, AX, AY, BX, BY, polygon[11], polygon[12],
                depth, dX1, 0.0001, 0.0001)
            end
          end
        end
      elseif clippingEnabled then
        ...
        if dX2 > 0.00010000001 then
          if dX3 > 0.00010000001 then
            ...
            if polygon[10] or (x2 - AX) * (y3 - y2) - (y2 - AY) * (x3 - x2) < 0 then
              buff:drawTriangle(AX, AY, x2, y2, x3, y3, polygon[11], polygon[12],
                depth, 0.0001, dX2, dX3)
              ...
              local BX = (newPosAZ * 10000) * sXFactor + renderOffsetX
              local BY = (newPosAY * 10000) * sYFactor + renderOffsetY
              buff:drawTriangle(AX, AY, BX, BY, x3, y3, polygon[11], polygon[12],
                depth, 0.0001, 0.0001, dX3)
            end
          else
            -- 0 1 0
            ...
            if polygon[10] or (x2 - AX) * (BY - y2) - (y2 - AY) * (BX - x2) < 0 then
              buff:drawTriangle(AX, AY, x2, y2, BX, BY, polygon[11], polygon[12],
                depth, 0.0001, dX2, 0.0001)
            end
          end
        else
  ...
  function frame:setFoV(FoV)
    ...
    self.camera[8] = 2 *
      math.atan(math.tan(rad(self.FoV) * 0.5) / (self.width / self.height / 1.5))
  end
  ...
  function frame:getObjectIndexTrace(objects, x, y)
    ...
    local function isInTriangle(checkX, checkY, x1, y1, x2, y2, x3, y3, framex1,
                                framey1, framex2, framey2)
    ...
    for i = 1, #objects do
      ...
      for j = 1, #model do
        ...
        if onScreen1 then
          ...
          if onScreen2 then
            local x3, y3, onScreen3 = map3dTo2d(polygon[7], polygon[8],
              polygon[9])
            if onScreen3 then
              if polygon[10] or (x2 - x1) * (y3 - y2) - (y2 - y1) * (x3 - x2) < 0 then
                local avgX = xCameraOffset +
                  (polygon[1] + polygon[4] + polygon[7]) / 3
                local avgY = yCameraOffset +
                  (polygon[2] + polygon[5] + polygon[8]) / 3
                local avgZ = zCameraOffset +
                  (polygon[3] + polygon[6] + polygon[9]) / 3

                local depth = avgX * avgX + avgY * avgY +
                  avgZ *
                  avgZ -- relative distance
...
function models:cube(options)
  ...
  ---@class Model
  local cube = {
    newPoly(-.5, -.5, -.5, .5, -.5, .5, -.5, -.5, .5,
      options.bottom or options.color),
    newPoly(-.5, -.5, -.5, .5, -.5, -.5, .5, -.5, .5,
      options.bottom2 or options.bottom or options.color),
    newPoly(-.5, .5, -.5, -.5, .5, .5, .5, .5, .5, options.top or options.color),
    newPoly(-.5, .5, -.5, .5, .5, .5, .5, .5, -.5, options.top or options.color),
    newPoly(-.5, -.5, -.5, -.5, -.5, .5, -.5, .5, -.5,
      options.side or options.color),
    newPoly(-.5, -.5, .5, -.5, .5, .5, -.5, .5, -.5,
      options.side2 or options.side or options.color),
    ...
    newPoly(-.5, -.5, -.5, -.5, .5, -.5, .5, .5, -.5,
      options.side2 or options.side or options.color),
    newPoly(-.5, -.5, .5, .5, -.5, .5, -.5, .5, .5, options.side or options
      .color),
    newPoly(.5, -.5, .5, .5, .5, .5, -.5, .5, .5,
      options.side2 or options.side or options.color),
  }
...
function models:icosphere(options)
  ...
  local function buildPoly(i1, i2, i3)
    return newPoly(v[i1][1], v[i1][2], v[i1][3], v[i2][1], v[i2][2], v[i2][3],
  ...
  local function subdivide()
    ...
    for i = 1, #model do
      ...
      newModel[#newModel + 1] = newPoly(AB.x, AB.y, AB.z, BC.x, BC.y, BC.z, AC.x,
        AC.y, AC.z, poly.c)
      newModel[#newModel + 1] = newPoly(poly.x1, poly.y1, poly.z1, AB.x, AB.y,
        AB.z, AC.x, AC.y, AC.z, nextColor)
      newModel[#newModel + 1] = newPoly(AB.x, AB.y, AB.z, poly.x2, poly.y2,
        poly.z2, BC.x, BC.y, BC.z, nextColor)
      newModel[#newModel + 1] = newPoly(AC.x, AC.y, AC.z, BC.x, BC.y, BC.z,
        poly.x3, poly.y3, poly.z3, nextColor)
    end
    model = newModel
  end
...
function models:plane(options)
  ...
  ---@class Model
  local plane = {
    newPoly(-1 * options.size, options.y, 1 * options.size, 1 * options.size,
      options.y, -1 * options.size, -1 * options.size, options.y,
...
function models:mountains(options)
  ...
  local model = {}
  for i = 0, options.res do
    local offset = math.random(-options.randomOffset * 100,
      options.randomOffset * 100) / 100
    local pos = i + offset
    ...
    local mountainHeight = math.random(minHeight * 100, maxHeight * 100) / 100 *
      options.scale
    ...
    if options.snow then
      ...
      local snowRatio = 1 -
        (realSnowRatio * maxHeight) / (mountainHeight / options.scale)
