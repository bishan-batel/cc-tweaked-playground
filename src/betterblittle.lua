...
local function getColorsFromPixelGroup(p1, p2, p3, p4, p5, p6)
  ...
  for c2 = 1, 16 do
    if c2 ~= c1 and freq[c2] > 0 then
      ...
      local err = min(c1Dists[p1], c2Dists[p1]) + min(c1Dists[p2], c2Dists[p2]) +
        min(c1Dists[p3], c2Dists[p3])
        + min(c1Dists[p4], c2Dists[p4]) + min(c1Dists[p5], c2Dists[p5]) +
        min(c1Dists[p6], c2Dists[p6])
...
local function sRGBtoOklab(r, g, b)
  local function f(v)
    return v >= 0.04045 and ((v + 0.055) / (1 + 0.055)) ^ 2.4 or
      (v / 12.92)
  end
  r, g, b = f(r), f(g), f(b)
...
local function drawBuffer(buffer, window, wx, wy)
  ...
  for y = 0, floor(height / 3) - 1 do
    ...
    local r2 = buffer
      [oy + 1]            -- second row from buffer for this row of characters
    local r3 = buffer
      [oy + 2]            -- third row from buffer for this row of characters
